...
const getDetailedStatusClass = (detailedStatus) => {
  if (!detailedStatus || detailedStatus === 'pending') {
    return 'detailed-status-pending';  // This applies the pending style for empty/null status
  }
  if (detailedStatus === 'ongoing') return 'detailed-status-ongoing';
...
// Update the getMainStatus function to be more explicit
const getMainStatus = (detailedStatus) => {
  if (!detailedStatus || detailedStatus === 'pending') {
    return 0; // Explicitly return Pending (0) for empty or null
  }
  ...
  return 0; // Default to Pending for any unmatched status
};

// Update the handleStatusChanged function to properly handle the empty/pending state
const handleStatusChanged = async (event) => {
  ...
  try {
    // Determine the main status based on the detailed status
    ...
    const mainResponse = await fetch(`http://193.203.162.228:5000/api/requests/${selectedRequest._id}`, {
      ...
      body: JSON.stringify(mainUpdatePayload),
    });
...
>
  ...
  <option value={3}>Canceled</option>
    ...
    <div className="modal4">
      ...
      <td>
      <select 
  value={selectedRequest?.detailedStatus || 'pending'} 
