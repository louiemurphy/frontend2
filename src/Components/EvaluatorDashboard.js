...
function EvaluatorDashboard() {
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      try {
        const response = await fetch(`http://193.203.162.228/api/teamMembers/${evaluatorId}`);
        if (!response.ok) throw new Error('Failed to fetch profile data');
        const data = await response.json();
        if (data.profileImage) setProfileImage(`http://193.203.162.228${data.profileImage}`);
      } catch (err) {
    ...
    const fetchRequests = async () => {
      try {
        const response = await fetch(`http://193.203.162.228/api/requests?assignedTo=${teamMember}`);
  ...
  const handleProfileUpload = async () => {
    if (profileFile) {
      ...
      try {
        const response = await fetch('http://193.203.162.228/api/uploadProfile', {
          method: 'POST',
          body: formData,
        });
        ...
        setProfileImage(`http://193.203.162.228${result.filePath}`);
  ...
  const handleStatusChange = async (requestId, newStatus, fileUrl) => {
    ...
    try {
      const response = await fetch(`http://193.203.162.228/api/requests/${requestId}`, {
        method: 'PUT',
  ...
  const handleFileUpload = async () => {
    if (uploadedFile && selectedRequest) {
      ...
      try {
        const response = await fetch('http://193.203.162.228/api/upload', {
          method: 'POST',
          body: formData,
        });
  ...
  const downloadFile = async (fileUrl, fileName) => {
    try {
      const response = await fetch(`http://193.203.162.228${fileUrl}`, {
        method: 'GET',
      });
