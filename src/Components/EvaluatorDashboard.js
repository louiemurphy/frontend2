...
function EvaluatorDashboard() {
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      try {
        const response = await fetch(`https://backend2-production-e5a1.up.railway.app/api/teamMembers/${evaluatorId}`);
        if (!response.ok) throw new Error('Failed to fetch profile data');
        const data = await response.json();
        if (data.profileImage) setProfileImage(`https://backend2-production-e5a1.up.railway.app${data.profileImage}`);
      } catch (err) {
    ...
    const fetchRequests = async () => {
      try {
        const response = await fetch(`https://backend2-production-e5a1.up.railway.app/api/requests?assignedTo=${teamMember}`);
  ...
  const handleProfileUpload = async () => {
    if (profileFile) {
      ...
      try {
        const response = await fetch('https://backend2-production-e5a1.up.railway.app/api/uploadProfile', {
          method: 'POST',
          body: formData,
        });
        ...
        setProfileImage(`https://backend2-production-e5a1.up.railway.app${result.filePath}`);
  ...
  const handleFileUpload = async () => {
    if (uploadedFile && selectedRequest) {
      ...
      try {
        const response = await fetch('https://backend2-production-e5a1.up.railway.app/api/upload', {
          method: 'POST',
          body: formData,
        });
