...
function AdminDashboard() {
  ...
  useEffect(() => {
    const fetchRequests = async () => {
      try {
        const response = await fetch('https://backend2-production-b1e6.up.railway.app/api/requests', { mode: 'cors' });
        if (!response.ok) {
  ...
  const handleSave = async () => {
    if (selectedTeamMember) {
      ...
      try {
        const response = await fetch(`https://backend2-production-b1e6.up.railway.app/api/requests/${selectedRequest._id}`, {
          method: 'PUT',
          headers: {
